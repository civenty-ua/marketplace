 ...
 */
class ImportServicesCategoriesCommand extends AbstractCategoriesImportCommand
{
    protected static $defaultName = 'app:import:market:categories-services';
    /**
     * @inheritDoc
     */
    protected function configure(): void
    {
        ...
        $this
            ->setDescription('Application data import: market services categories')
            ->setHelp('Run Application data import process for market services categories')
            ->addArgument(
                'file',
                InputArgument::REQUIRED,
                'path to source file, absolute or from application root '.
                '(for example: src/DataFixtures/sources/import/market/servicesCategories/data.xlsx)'
            );
    }
    /**
     * @inheritDoc
    ...
    protected function parseDataProviderFile(SplFileInfo $file): array
    {
        ...
        foreach ($data as $category => $sheetData) {
            $noSubCategory = !empty($sheetData[0][0]) && !empty($sheetData[1][0]);
            ...
            if (!$noSubCategory) {
                ...
                $attributesData = $this->parseSheetDataWithNoSubcategories($sheetData);
                ...
                $result[] = [
                    'category'      => $this->normalizeTitleValue($category),
                    'parent'        => null,
                    'attributes'    => $attributesData,
                ];
            }
    ...
    /**
     * Parse sheet data (subcategories NOT data exist!).
     *
     * @param   array $sheetData            Sheet data.
     *
     * @return  array                       Parsed data.
     */
    private function parseSheetDataWithNoSubcategories(array $sheetData): array
    {
        $result = [];
        ...
        foreach ($sheetData as $rowIndex => $row) {
            if ($rowIndex === 0) {
                ...
                foreach ($row as $cellIndex => $cell) {
                    if (!empty($cell) && isset($result[$cellIndex])) {
                        $result[$cellIndex]['values'][] = $cell;
                    }
                }
            }
        }
    ...
    {
        $category = $this->getCategory($item['category'], Commodity::TYPE_SERVICE);

        if ($item['parent']) {
            $categoryParent = $this->getCategory($item['parent'], Commodity::TYPE_SERVICE);
